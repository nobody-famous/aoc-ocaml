...
let solve grid =
  grid.graph |> G.shortest_path { start_pos = grid.s; initial_weight = 0; is_end = (fun p -> p = grid.e) } |> fun n ->
  n.weight

let run lines = Aoc.Utils.IntResult (parse_input lines |> build_grid to_edge |> solve)
