...
let slope_to_quadrant slope point origin =
  if (slope < 0. || slope = Float.infinity) && point.y < origin.y then
    NORTH_EAST
  else if slope >= 0. && point.x > origin.x then
    SOUTH_EAST
    ...
    failwith "Could not assign quadrant"
...
let compare_slopes s1 s2 =
  if s1.quad = s2.quad then
    if s1.slope = Float.infinity then
      -1
    else if s2.slope = Float.infinity then
      1
    else if s1.slope = Float.infinity then
      -1
    else
      compare s1.slope s2.slope
  else
    compare_quadrant s1.quad s2.quad

let map_to_array (origin, map) =
  let point_array =
    Hashtbl.to_seq map |> Seq.fold_left (fun acc (slope, sides) -> add_sides acc slope sides origin) [] |> Array.of_list
  in
...
let sum_asteroids arr = Array.fold_left (fun acc s -> acc + List.length s.points) 0 arr
...
let remove_asteroids arr =
  ...
  let rec loop ndx count last =
    if count = total then
      ...
      last
    else if ndx >= Array.length arr then
      loop 0 count last
    else
      let item = arr.(ndx) in
